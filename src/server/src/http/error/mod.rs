...
/// A Result where the error type is automatically deferred to a HTTP error.
...
pub enum HttpError {
    ...
    /// Automatically return `500 Internal Server HttpError` on a `sqlx::Error`.
    ///
    /// Via the generated `From<sqlx::Error> for HttpError` impl,
    ...
    ///
    /// Via the generated `From<anyhow::Error> for HttpError` impl, this allows the
    ...
    fn into_response(self) -> Response<Body> {
        match self {
            Self::UnprocessableEntity { errors } => {
                ...
                return (StatusCode::UNPROCESSABLE_ENTITY, Json(HttpErrors { errors })).into_response();
            }
            Self::Unauthorized => {
                return (
                    self.status_code(),
...
///     .fetch_one(&ctxt.db)
///     .await
///     .on_constraint("user_username_key", |_| HttpError::unprocessable_entity([("username", "already taken")]))?;
/// ```
///
